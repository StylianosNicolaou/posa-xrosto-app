...
export function CurrencySelector({ currency, onCurrencyChange }: CurrencySelectorProps) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48 rounded-xl p-2 bg-white border-neutral-200">
        {Object.values(CURRENCIES).map((curr) => (
          <DropdownMenuItem
            key={curr.code}
            onClick={() => onCurrencyChange(curr.code)}
            className={`cursor-pointer py-2.5 rounded-lg text-neutral-700 focus:bg-neutral-100 focus:text-neutral-900 ${
